...
import { Helmet } from "react-helmet";
    ...
    const BlogTitle = React.forwardRef(({ title, library, institute,onClick, location }, ref) => {
    ...
    const CollegePost = () => {
      ...
      const handleSearchChange = (event) => {
        ...
        navigate(`/institute/search/${encodeURIComponent(newQuery)}`);
      };


      const fetchDataForinstitute = async (institute) => {
        try {
          const response = await fetch(`https://apiforcollegeblogs-t63y.onrender.com//api/${institute}`);
      ...
      useEffect(() => {
        if (institute) {
          fetchDataForinstitute(institute).then(() => setLoading(false));
        } else {
      ...
      const filteredBlogs = (institute) => {
        const blogslibrary = blogsData[institute] || [];
  ...
  fetchDataForAlllibrarys().then(() => setLoading(false));
      ...
      useEffect(() => {
        const query = location.pathname.split("/institute/search/")[1] || "";
        ...
        const urlTitleMatch = location.pathname.match(/\/institute\/(.+?)\/(.+)/);
        if (urlTitleMatch) {
      ...
      const currentPosts = filteredBlogs(institute).slice(indexOfFirstPost, indexOfLastPost);
            ...
            element = (
              <VStack key={index} align="start" spacing={2} mt={2}>
                <BlogTitle
                  title={item.title}
                  library={institute}
                  onClick={() => handleTitleClick(item.title, institute)}
                />
            ...
            <>
{
                  ...
                  <motion.div
                    ...
                    ref={
                      index === currentPosts.length - 1
                        ? (node) => observeLastBlog(institute, node)
                        : null
                    }
                  >
  ...
  <BlogTitle
    ...
    title={blog.title}
    library={institute}
    onClick={() => handleTitleClick(blog.title, institute)}
    location="main" 
  />
